...
export default function EmployeeLogin() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await login(email, password, true); // true for isEmployee
      navigate('/admin');
    } catch (err) {
    ...
    <div className="flex-grow flex items-center justify-center bg-slate-950 py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      >
        <div className="flex flex-col items-center text-center">
          ...
          <p className="mt-3 text-sm text-slate-400"> Authorized personnel only </p>
        </div>
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-1">Employee Email</label>
              <input
                type="email"
                required
                ...
                placeholder="admin@riskandtonic.com"
                value={email}
